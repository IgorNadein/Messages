...
public class Headers {
    ...
    public static Headers deSerializeHeader(byte[] serializedHeader) throws NumberFormatException {
        if(serializedHeader == null || serializedHeader.length != SERIALIZED_HEADER_SIZE)
            throw new NumberFormatException("Invalid ratchet header length");
    ...
    public byte[] getSerialized() throws IOException {
        if(this.dh == null || this.dh.length != PUBLIC_KEY_SIZE)
            throw new IOException("Invalid ratchet public key");
        if(this.PN < 0 || this.N < 0)
            throw new IOException("Invalid ratchet counter");
        byte[] bytesPN = new byte[4];
    ...
    private static final int SERIALIZED_HEADER_SIZE = 8 + PUBLIC_KEY_SIZE;
}
