...
public class Ratchets {
    ...
    private static byte[] trySkippedMessageKeys(States state, Headers header, byte[] cipherText, byte[] AD) throws Throwable {
        Pair<byte[], Integer> matchedKey = null;
        for(Pair<byte[], Integer> candidate : state.MKSKIPPED.keySet()) {
            if(candidate.second == header.N && Arrays.equals(candidate.first, header.dh)) {
                matchedKey = candidate;
                break;
            }
        }
        if(matchedKey != null){
            byte[] mk = state.MKSKIPPED.remove(matchedKey);
            try {
                return Protocols.DECRYPT(mk, cipherText, Protocols.CONCAT(AD, header));
            } finally {
    ...
    private static void skipMessageKeys(States state, int until) throws Exception {
        ...
        if(state.CKr != null) {
            while(state.Nr < until) {
                ...
                byte[] mk = kdfCkOutput.second;
                state.MKSKIPPED.put(
                        new Pair<>(Arrays.copyOf(state.DHr, state.DHr.length), state.Nr), mk);
                state.Nr +=1;
            }
        }
    }
