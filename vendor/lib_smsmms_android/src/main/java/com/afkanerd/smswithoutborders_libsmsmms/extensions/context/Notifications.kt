...
fun Context.notify(
    ...
    val participants = notificationParticipants(conversation)
    val conversationTitle = notificationConversationTitle(participants)
    ...
    if(actions) {
        ...
        messages?.sortedWith(compareBy {it.date})?.forEach {
            val senderAddress = it.senderAddress
                ?.takeIf(String::isNotBlank)
                ?: participants.singleOrNull()
            val senderName = senderAddress?.let(::retrieveContactName) ?: senderAddress
            val sender = senderAddress?.let { address ->
                Person.Builder()
                    .setName(senderName ?: address)
                    .setKey(address)
                    .setImportant(true)
                    .build()
            }
            style.addMessage(
                (if(Build.VERSION.SDK_INT < Build.VERSION_CODES.P) {
                    NotificationCompat.MessagingStyle.Message(
                        text ?: it.text,
                        it.date,
                        if(it.self) user.name else senderName
                    )
                } else {
                    NotificationCompat.MessagingStyle.Message(
                        text ?: it.text,
        ...
        if(isGroupConversation) style.conversationTitle = title ?: conversationTitle
        builder.setStyle(style)
    }
    else {
        builder.setContentTitle(title ?: conversationTitle)
...
fun Context.getNotificationBuilder(
    ...
    val participants = notificationParticipants(conversation)
    val conversationTitle = notificationConversationTitle(participants)
    val senderAddress = conversation.sender_address
        ?: participants.singleOrNull()
        ?: conversation.sms!!.address!!
    val contactName = retrieveContactName(senderAddress)
    val sender = Person.Builder()
        .setName(contactName ?: senderAddress)
    ...
    return NotificationCompat.Builder(
        ...
        .apply {
            if(actions) {
                if(participants.none(::isShortCode)) {
...
private fun Context.insertNotificationSessions(conversation: Conversations, self: Boolean) {
    ...
    with(sharedPreferences.edit()) {
        ...
        val notifSession = IncomingNotificationSession(
            ...
            senderAddress = conversation.sender_address,
        )
...
private fun Context.notificationParticipants(conversation: Conversations): List<String> {
    val threadId = conversation.sms?.thread_id ?: return emptyList()
    val stored = getDatabase().conversationsDao()
        ?.getThreadParticipantAddresses(threadId)
        .orEmpty()
    if(stored.isNotEmpty()) return stored
    return conversation.sms?.address.orEmpty()
        .split(',')
        .map(String::trim)
...
private fun Context.notificationConversationTitle(participants: List<String>): String =
    participants.joinToString(", ") { address -> retrieveContactName(address) ?: address }
