...
class CloudDownloadWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        return if(AttachmentManager.get(applicationContext).resumeCloudDownload(transferId)) {
            Result.success()
        } else {
            Result.retry()
        }
    ...
    companion object {
        private const val EXTRA_TRANSFER_ID = "cloud_transfer_id"
        ...
        fun cancel(context: Context, transferId: String) {
            WorkManager.getInstance(context).cancelUniqueWork("cloud-download-$transferId")
        }
    }
}
